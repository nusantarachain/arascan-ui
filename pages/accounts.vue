    ...
    <fvTable id="Accounts" :fields="fields" :rows="rows" class="accounts">
      <template v-slot:field="{ field, row }">
        ...
        <span v-if="field == 'Address'">
          <NuxtLink
            class="address fv-hidden-sm fv-hidden-xs fv-hidden-md"
            :to="`/account/${row[field]}`"
            >{{ row[field] }}</NuxtLink
          >
          <NuxtLink
            class="address fv-hidden-lg fv-hidden-xl"
            :to="`/account/${row[field]}`"
          >
            {{ row[field + "_short"] }}
          </NuxtLink>
        </span>
        <span v-if="field == 'Balance'"> {{ row[field] }} ARA </span>
